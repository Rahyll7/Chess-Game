...
import {
  ...
  handleLongCastling,
  promotePawns,
} from "../../redux/piecesReducer/actions";
...
function ChessBoard() {
  ...
  function checkIfmoveAllowedForEscapeCheckMate(
    ...
    if (type === "king") {
      ...
      let checkDefendersAndEaters = Object?.keys(defendersAndEaters).find(
        (item) => {
  ...
  function checkCastlingKingNewSquare(kingNewSquare, castlingType) {
    ...
    let filteredArray = concatedArray?.filter(
      (item) => !item.includes("0") && !item.includes("9") && item.length <= 2
    );
    let foundInEnemyAllowedMoves = filteredArray.some(
      (item) => item === kingNewSquare
    );

    if (castlingType === "short") {
      if (
        pieces[parseInt(kingCol) + 1 + kingRow] === undefined &&
  ...
  function castling() {
    ...
    if (
      ...
      (pieces[firstSelected.current]?.color ===
        pieces[secondSelected]?.color) ===
        currentPiece?.color &&
      (firstSelected.current === currentPiece?.color) === "white"
        ? "51"
        : "58" && (secondSelected === currentPiece?.color) === "white"
        ? "81"
        : "88") ||
      ...
      (firstSelected.current === currentPiece?.color) === "white"
        ? "81"
        : "88" && (secondSelected === currentPiece?.color) === "white"
        ? "51"
        : "58")
    ) {
      if (
        (pieces[secondSelected]?.type === "rook" && secondSelected === "81") ||
        (pieces[firstSelected.current]?.type === "rook" &&
          firstSelected.current === "81") ||
        (pieces[secondSelected]?.type === "rook" && secondSelected === "88") ||
        (pieces[firstSelected.current]?.type === "rook" &&
          firstSelected.current === "88")
      ) {
        if (isCheckMate.black === false && isCheckMate.white === false) {
          if (
            checkCastlingKingNewSquare(
              currentPiece?.color === "white" ? "71" : "78",
              "short"
            )
          ) {
            dispatch(
    ...
    if (
      ...
      (pieces[firstSelected.current]?.color ===
        pieces[secondSelected]?.color) ===
        currentPiece?.color &&
      secondSelected === (currentPiece?.color === "white" ? "51" : "58") &&
      (firstSelected.current === currentPiece?.color) === "white"
        ? "11"
        : "18") ||
      ((pieces[firstSelected.current]?.type === "king" &&
      ...
      (pieces[firstSelected.current]?.color ===
        pieces[secondSelected]?.color) ===
        currentPiece?.color &&
      (secondSelected === currentPiece?.color) === "white"
        ? "11"
        : "18") && (firstSelected.current === currentPiece?.color) === "white"
        ? "51"
        : "58")
    ) {
      if (
        (pieces[secondSelected]?.type === "rook" && secondSelected === "11") ||
        (pieces[firstSelected.current]?.type === "rook" &&
          firstSelected.current === "11") ||
        (pieces[secondSelected]?.type === "rook" && secondSelected === "18") ||
        (pieces[firstSelected.current]?.type === "rook" &&
          firstSelected.current === "18")
      ) {
        if (isCheckMate.black === false && isCheckMate.white === false) {
          if (
            checkCastlingKingNewSquare(
              currentPiece?.color === "white" ? "31" : "38",
              "long"
            )
          ) {
  ...
  function firstSelectedPiece(col, row, square) {
    ...
    if (checkPlayerTurn(col, row, playerTurn, pieces)) {
      ...
      if (isCheckMate.black === true || isCheckMate.white === true) {
        let allowed = checkIfmoveAllowedForEscapeCheckMate(
          checkMateAllowedMoves,
          currentPiece,
          currentSquare,
          pieces[square].type
        ...
        if (allowed) {
          ...
          setCurrentPiece({});
          Notification(
            "It's Check Mate, Choose a valid piece to play !!",
            "red"
          );
        }
      }
    ...
    <>
      {isGameOver && (
        <div className="gameOver">
          ...
          <br />
          <p className="winner">
            {isCheckMate.black === true ? "White" : "Black"} Won
          </p>
        </div>
      )}
      {couldBePromoted && (
        <Promotion
          couldBePromoted={setCouldBePromoted}
        ...
        <div className="Trash">
          {piecesTrash.map((piece) => {
            if (piece?.color === "white") {
              return (
                <img
                  src={`images/${piece?.color}/${piece?.type}.svg`}
                  className={"trashPieces"}
                  alt="type"
                />
              );
            }
